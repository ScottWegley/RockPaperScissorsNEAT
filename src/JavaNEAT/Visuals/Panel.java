...
public class Panel extends JPanel {
    ...
    protected void paintComponent(Graphics g) {
        ...
        for (ConnectionGene c : genome.getConnections().getList()) {
            paintConnection(c, (Graphics2D) g);
        }


        for (NodeGene n : genome.getNodes().getList()) {
    ...
    private void paintConnection(ConnectionGene c, Graphics2D g) {
        g.setColor(c.isEnabled() ? Color.white : Color.gray);
        g.setStroke(new BasicStroke(3));
        ...
        g.drawLine((int) (this.getWidth() * c.getFrom().getX()),
